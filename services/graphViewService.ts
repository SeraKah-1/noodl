...
export function buildGraphHtmlLocal(graphData: GraphData, title: string): string {
  ...
  const payload = JSON.stringify({
    ...
    labels: {
      ...
      hint: isId ? 'Hover untuk ringkasan · klik untuk detail' : 'Hover for a summary · click for details',
      core: isId ? 'Inti' : 'Core',
...
<style>
  ...
  *{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;min-height:0;overflow:hidden;font:14px/1.5 Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;color:var(--ink);background:#f8fafc}button,input{font:inherit}
  ...
  .map-card{display:grid;grid-template-rows:auto minmax(250px,1fr)}.map-toolbar{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 14px;border-bottom:1px solid var(--line)}
  ...
  .stage{position:relative;min-height:250px;overflow:hidden;background:radial-gradient(circle at 48% 43%,#fff 0,#f8f7ff 45%,#eef2ff 100%)}.stage:before{content:"";position:absolute;inset:0;background-image:radial-gradient(#c7d2fe 1px,transparent 1px);background-size:24px 24px;opacity:.45;pointer-events:none}
...
<main class="app">
  ...
  <div class="layout">
    <section class="map-card"><div class="map-toolbar"><div class="stats" id="stats"></div><div class="hint" id="hint"></div></div><div class="stage" id="stage"><svg id="graph" aria-label="Knowledge graph"></svg><div class="tooltip" id="tooltip"><strong></strong><span></span></div></div></section>
...
function selectNode(id){selectedId=id;reviewIndex=0;hideTooltip();renderGraph();renderReview()}
function renderGraph(){const nodes=visibleNodes();const pos=positions(nodes);const rings=Math.max(1,Math.ceil((nodes.length-1)/8));const extent=Math.max(215,125+rings*105);graph.setAttribute('viewBox',[-extent,-extent,extent*2,extent*2].join(' '));graph.replaceChildren();
