...
export const MaterialOverviewModal: React.FC<MaterialOverviewModalProps> = ({
  questions,
  result,
  title,
  quizId,
      ...
      <div className="bg-slate-50 dark:bg-slate-900 w-full max-w-6xl h-[calc(100dvh-1rem)] sm:h-[min(90dvh,56rem)] rounded-2xl sm:rounded-3xl flex flex-col shadow-2xl relative overflow-hidden border border-white/10">
         <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center gap-2 bg-white dark:bg-slate-950 sticky top-0 z-20 shadow-sm">
            <h2 id="material-overview-title" className="text-lg font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2 min-w-0">
               <span className="text-indigo-600 shrink-0">( •_•)⌐■-■</span>
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              <button type="button" aria-label="Close concept map" onClick={onClose} className="p-2 text-slate-500 hover:bg-rose-50 hover:text-rose-600 rounded-lg transition-colors">
                 <X size={20} />
              </button>
            </div>
         </div>
