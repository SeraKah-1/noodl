  ...
  const startQuizGeneration = async (files: File[] | null, config: ModelConfig) => {
    ...
    if (!apiKey && !isAiAvailableWithoutUserKey) {
      showErrorNotification({
        ...
        error: "API Key missing for Gemini",
        possibleCauses: [
          "API Key Gemini belum diisi di menu Settings.",
            ...
            <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl border-2 border-indigo-200 dark:border-indigo-700 rounded-2xl p-5 shadow-2xl shadow-indigo-500/20">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-slate-800 dark:text-slate-100 text-sm">Quiz Belum Selesai</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    Kamu punya sesi quiz yang belum selesai ({pendingSession.session.answers?.length || 0} soal sudah dijawab).
                  </p>
